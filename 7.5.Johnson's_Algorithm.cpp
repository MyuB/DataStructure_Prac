...
bool HasNegativeCycle(const vector<Edge>& edges, vector<int> distance) {
	for (auto& e : edges) {
		...
		if (distance[e.src] + e.weight < distance[e.dst]) return true;
	}
	return false;
}
...
vector<int> Dijkstra(vector<Edge> edges, int V, int start) {
	...
	for (int i = 0; i < V - 1; i++) {
		...
		for (auto&e : edges) {
			...
			if (distance[curr] != UNKNOWN && distance[e.dst] > distance[e.src] + e.weight) {
				distance[e.dst] = distance[e.src] + e.weight;
			}
		}
	}
...
void Johnson(vector<Edge> edges, int V) {
	//더미 정점을 추가한 그래프에서 최단 거리 계산;
	vector<int> h = BellmanFord(edges, V);

	if (h.empty()) return;

	for (auto& e : edges) e.weight += (h[e.src] - h[e.dst]);
